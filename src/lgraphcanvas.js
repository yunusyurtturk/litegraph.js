...
export class LGraphCanvas {
    ...
    _doNothing(e) {
        // DBG LiteGraph.log_debug("pointerevents: _doNothing "+e.type);
        e.preventDefault();
        return false;
    }
    ...
    */
    ...
    processUserInputDown(e) {
        ...
        if (e.button !== undefined) {
            ...
            // DBG LiteGraph.log_debug("input button ",e.button);
            switch(e.button) {
        ...
        if (e.buttons !== undefined) {
            ...
            // DBG LiteGraph.log_debug("input button_S ",e.buttons);
        }
    ...
    processMouseDown(e) {
        ...
        var x = e.clientX;
        var y = e.clientY;
        // DBG LiteGraph.log_debug(y,this.viewport);
        ...
        if (this.pointer_is_down && is_primary ) {
            ...
            // DBG LiteGraph.log_debug("pointerevents: pointer_is_double start");
        }else{
            this.pointer_is_double = false;
        }
        ...
        if (e.which == 1 && !this.userInput_isNotPrimary) {
            ...
            if (LiteGraph.alt_drag_do_clone_nodes && e.altKey && node && this.allow_interaction && !skip_action && !this.read_only) {
                ...
                if (cloned) {
                    ...
                    if( LiteGraph.alt_shift_drag_connect_clone_with_input && e.shiftKey ) {
                        // process links
                        // DBG LiteGraph.log_debug("altCloned",original_node,node);
                        if (original_node.inputs && original_node.inputs.length) {
                            ...
                            for (var j = 0; j < original_node.inputs.length; ++j) {
                                ...
                                if (!input || input.link == null) {
                                    // DBG LiteGraph.log_debug("not input link",input);
                                    continue;
                                }
                                ...
                                if (ob_link.type === LiteGraph.EVENT) {
                                    // DBG LiteGraph.log_debug("skip moving events :: TODO put a sequencer in the middle or implement multi input",input);
                                    continue;
                                }
                                // DBG LiteGraph.log_debug("find link node",ob_link);
                                var source_node;
            ...
            if (node && (this.allow_interaction || node.flags.allow_interaction) && !skip_action && !this.read_only) {
                ...
                if (!skip_action) {
                    ...
                    if ( cbRet!==null && (cbRet === true || (typeof(cbRet)=="object" && cbRet.block_drag_node)) ) {
            ...
            if (LiteGraph.middle_click_slot_add_default_node) {
                if (node && this.allow_interaction && !skip_action && !this.read_only) {
                    ...
                    if (
                        ...
                        // DBG LiteGraph.log_debug("middleClickSlots? "+mClikSlot+" & "+(mClikSlot_index!==false));
                ...
                // DBG LiteGraph.log_debug("pointerevents: dragging_canvas start from middle button");
                this.dragging_canvas = true;
            }
    ...
    processMouseMove(e) {
        ...
        // DBG LiteGraph.log_debug("pointerevents: processMouseMove "+e.pointerId+" "+e.isPrimary);

        if(this.block_click) {
            // DBG LiteGraph.log_debug("pointerevents: processMouseMove block_click");
            e.preventDefault();
            return false;
        }
        ...
        if (this.dragging_rectangle) {
            ...
            if (this.node_dragged && !this.live_mode) {
                // DBG LiteGraph.log_debug("draggin!",this.selected_nodes);
    ...
    processMouseUp(e) {
        ...
        if(!is_primary) {
            /* e.stopPropagation();
            e.preventDefault();*/
            // DBG LiteGraph.log_debug("pointerevents: processMouseUp pointerN_stop "+e.pointerId+" "+e.isPrimary);
            return false;
        }

        // DBG LiteGraph.log_debug("pointerevents: processMouseUp "+e.pointerId+" "+e.isPrimary+" :: "+e.clientX+" "+e.clientY);
        ...
        if(!this.options.skip_events) {
            // DBG LiteGraph.log_debug("pointerevents: processMouseUp adjustEventListener");
        ...
        if(this.block_click) {
            // DBG LiteGraph.log_debug("pointerevents: processMouseUp block_clicks");
            this.block_click = false; // used to avoid sending twice a click in a immediate button
        }

        // DBG LiteGraph.log_debug("pointerevents: processMouseUp which: "+e.which);
        ...
        */
        ...
        // DBG LiteGraph.log_debug("pointerevents: processMouseUp stopPropagation");
        e.stopPropagation();
        e.preventDefault();
        return false;
    }
    ...
    processKey = (e) => {
        ...
        // DBG LiteGraph.log_debug(e); //debug
    ...
    adjustMouseEvent(e) {
        ...
        // DBG LiteGraph.log_debug("pointerevents: adjustMouseEvent "+e.clientX+":"+e.clientY+" "+clientX_rel+":"+clientY_rel+" "+e.canvasX+":"+e.canvasY);
    }
    ...
    processNodeWidgets(node, pos, event, active_widget) {
        if (!node.widgets || !node.widgets.length || (!this.allow_interaction && !node.flags.allow_interaction)) {
            if(!node.widgets || !node.widgets.length) LiteGraph.log_debug("graph processNodeWidgets","no widgets for node", node);
            if(!this.allow_interaction && !node.flags.allow_interaction) LiteGraph.log_debug("graph processNodeWidgets","interaction not allowed on graph and not overridden on node", node);
            return null;
        }
        ...
        for (let i = 0; i < node.widgets.length; ++i) {
            ...
            LiteGraph.log_debug("graph processNodeWidgets","has widget", w);
                ...
                case "button":
                    if (event.type === "pointerdown") {
                        if (w.callback) {
                            ...
                            LiteGraph.log_debug("graph processNodeWidgets","button, has not callback", w);
                        }
                        w.clicked = true;
                        this.dirty_canvas = true;
                    }else{
                        LiteGraph.log_debug("graph processNodeWidgets","button, event is not pointer down", event);
                    }
                    break;
                case "slider":
    ...
    */
    ...
    static showMenuNodeOptionalInputs(v, options, e, prev_menu, node) {
        ...
        if (!entries.length) {
            if (LiteGraph.debug)
                LiteGraph.log_debug("no input entries");
            return;
        }
            ...
            switch (v) {
                case "Add Node":
                    LGraphCanvas.onMenuAdd(null, null, e, menu, function(node) {
                        LiteGraph.log_debug("node autoconnect");
    ...
    createDefaultNodeForSlot(optPass = {}) { // addNodeMenu for connection
        ...
        if (!isFrom && !isTo) {
            LiteGraph.log_warn("No data passed to createDefaultNodeForSlot "+opts.nodeFrom+" "+opts.slotFrom+" "+opts.nodeTo+" "+opts.slotTo);
            return false;
        }
        if (!opts.nodeType) {
            LiteGraph.log_warn("No type to createDefaultNodeForSlot");
            return false;
        }
        ...
        switch (typeof slotX) {
            ...
            default:
                // bad ?
                // iSlotConn = 0;
                LiteGraph.log_warn("Cant get slot information "+slotX);
                return false;
        }

        if (slotX===false || iSlotConn===false) {
            LiteGraph.log_warn("createDefaultNodeForSlot bad slotX "+slotX+" "+iSlotConn);
        }
            ...
            if(typeof slotTypesDefault[fromSlotType] == "object") {
                for(var typeX in slotTypesDefault[fromSlotType]) {
                    if (opts.nodeType == slotTypesDefault[fromSlotType][typeX] || opts.nodeType == "AUTO") {
                        ...
                        // DBG LiteGraph.log_debug("opts.nodeType == slotTypesDefault[fromSlotType][typeX] :: "+opts.nodeType);
                        break; // --------
                    }
                }
            } else {
            ...
            if (nodeNewType) {
                ...
                if(newNode) {
                    ...
                    */
                    ...
                    LiteGraph.log_warn("failed creating "+nodeNewType);
                }
            }
        }
        return false;
    ...
    showConnectionMenu(optPass = {}) { // addNodeMenu for connection
        ...
        if (!isFrom && !isTo) {
            LiteGraph.log_warn("No data passed to showConnectionMenu");
            return false;
        }
    ...
    prompt(title = "", value, callback, event, multiline) {
        ...
        input.addEventListener("keydown", (e) => {
            ...
            switch (e.keyCode) {
                ...
                case 13: // Enter key
                    ...
                    LiteGraph.log_debug("prompt v2 ENTER",input.value,e.target.localName,callback);
                    dialog.close();
                    break;
                default:
        ...
        button.addEventListener("click", (_event) => {
            ...
            LiteGraph.log_debug("prompt v2 OK",input.value,callback);
            dialog.close();
        });
    ...
    showSearchBox(event, options) {
        ...
        // DBG LiteGraph.log_debug(options);
            ...
            if (selIn) {
                ...
                for (let iK=0; iK<nSlots; iK++) {
                    ...
                    if(options.type_filter_in !==false && (options.type_filter_in+"").toLowerCase() == (aSlots[iK]+"").toLowerCase()) {
                        // selIn.selectedIndex ..
                        opt.selected = true;
                        // DBG LiteGraph.log_debug("comparing IN "+options.type_filter_in+" :: "+aSlots[iK]);
                    }else{
                        // DBG LiteGraph.log_debug("comparing OUT "+options.type_filter_in+" :: "+aSlots[iK]);
                    }
                }
                ...
                if (that.onSearchBoxSelection) {
                    ...
                    if (options.node_from) {
                        ...
                        if (typeof options.node_from.outputs[iS] !== "undefined") {
                            ...
                            // DBG LiteGraph.log_warn("cant find slot " + options.slot_from);
                        }
                    }
                    if (options.node_to) {
                        iS = false;
                        ...
                        if (typeof options.node_to.inputs[iS] !== "undefined") {
                            ...
                            // DBG LiteGraph.log_warn("cant find slot_nodeTO " + options.slot_from);
                        }
                    }
            ...
            if (that.onSearchBox) {
                ...
                function inner_test_filter(type, optsIn = {}) {
                    ...
                    if(options.do_type_filter && !opts.skipFilter) {
                        ...
                        if(sIn && sV) {
                            // DBG LiteGraph.log_debug("will check filter against "+sV);
                            if (LiteGraph.registered_slot_in_types[sV] && LiteGraph.registered_slot_in_types[sV].nodes) { // type is stored
                                // DBG LiteGraph.log_debug("check "+sType+" in "+LiteGraph.registered_slot_in_types[sV].nodes);
                                doesInc = LiteGraph.registered_slot_in_types[sV].nodes.includes(sType);
                                if (doesInc!==false) {
                                    // DBG LiteGraph.log_debug(sType+" HAS "+sV);
                                }else{
                                    // DBG LiteGraph.log_debug(LiteGraph.registered_slot_in_types[sV]," DONT includes "+type);
                                    return false;
                                }
                            }
                        }
                        ...
                        if(sOut && sV) {
                            // DBG LiteGraph.log_debug("search will check filter against "+sV);
                            if (LiteGraph.registered_slot_out_types[sV] && LiteGraph.registered_slot_out_types[sV].nodes) { // type is stored
                                // DBG LiteGraph.log_debug("check "+sType+" in "+LiteGraph.registered_slot_out_types[sV].nodes);
                                doesInc = LiteGraph.registered_slot_out_types[sV].nodes.includes(sType);
                                if (doesInc!==false) {
                                    // DBG LiteGraph.log_debug(sType+" HAS "+sV);
                                }else{
                                    // DBG LiteGraph.log_debug(LiteGraph.registered_slot_out_types[sV]," DONT includes "+type);
                                    return false;
                                }
                            }
                        }
    ...
    showEditPropertyValue(node, property, options) {
        ...
        if ((type == "enum" || type == "combo") && info.values) {
            LiteGraph.log_debug("showEditPropertyValue ENUM COMBO",input,type,dialog);
            input = dialog.querySelector("select");
            input.addEventListener("change", function(e) {
                dialog.modified();
                LiteGraph.log_debug("Enum change",input,info,e.target);
                setValue(e.target.value);
                // var index = e.target.value;
                // setValue( e.options[e.selectedIndex].value );
            });
        } else if (type == "boolean" || type == "toggle") {
            LiteGraph.log_debug("showEditPropertyValue TOGGLE",input,type,dialog);
            ...
            input = dialog.querySelector("input");
            LiteGraph.log_debug("showEditPropertyValue",input,type,dialog);
            if (input) {
                input.addEventListener("blur", function(_event) {
                    this.focus();
                });
    ...
    createPanel(title, options) {
        ...
        root.addWidget = function( type, name, value, options, callback ) {
            ...
            LiteGraph.log_debug("addWidget",type,value,value_element,options);
            ...
            if(type == "code") {
                ...
                LiteGraph.log_debug("addWidget ENUM COMBO",type,str_value,value_element,options);
            ...
            function innerChange(name, value) {
                LiteGraph.log_debug("widgetInnerChange",name,value,options);
                // that.dirty_canvas = true;
                if(options.callback)
            ...
            const fUpdate = (name, value, options) => {
                switch(name) {
                    /* case "Render mode":
                        ...
                        break;*/
                    default:
                        // DBG LiteGraph.log_debug("want to update graph options: "+name+": "+value);
                        ...
                        // DBG LiteGraph.log_debug("update graph option: "+name+": "+value);
                ...
                switch(name) {
                    ...
                    case "Mode":
                        ...
                        if (kV>=0 && LiteGraph.NODE_MODES[kV]) {
                            ...
                            LiteGraph.log_warn("unexpected mode: "+value);
                        }
                        break;
                    case "Color":
                        if (LGraphCanvas.node_colors[value]) {
                            ...
                            LiteGraph.log_warn("unexpected color: "+value);
                        }
                        break;
                    default:
    ...
    showSubgraphPropertiesDialog(node) {
        LiteGraph.log_debug("showing subgraph properties dialog");
    ...
    showSubgraphPropertiesDialogRight(node) {
        ...
        // DBG LiteGraph.log_debug("showing subgraph properties dialog");
        ...
        function inner_clicked(v) {
            ...
            const fApplyMultiNode = (node) => {
                if (kV>=0 && LiteGraph.NODE_MODES[kV])
                    ...
                    LiteGraph.log_warn("unexpected mode: "+v);
