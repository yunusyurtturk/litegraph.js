...
(function(global) {
    ...
    var LiteGraph = (global.LiteGraph = {
        ...
        context_menu_filter_enabled: true,

        showCanvasOptions: false, //[true!] customize availableCanvasOptions
        ...
        ensureUniqueExecutionAndActionCall: true, // NEW ensure single event execution
        ...
        registerNodeType: function(type, base_class) {
            ...
            if (LiteGraph.onNodeTypeRegistered) {
                LiteGraph.onNodeTypeRegistered(type, base_class); // tag: litegraph event entrypoint
            }
            if (prev && LiteGraph.onNodeTypeReplaced) {
                LiteGraph.onNodeTypeReplaced(type, base_class, prev); // tag: litegraph event entrypoint
            }
        ...
        */
        ...
        // purposes of debug: reloads all the js scripts that matches a wildcard
    ...
    */
    ...
    LGraph.prototype.getAncestors = function(node, optsIn) {
        ...
        //DBG("---getAncestors--- for "+node.id+":"+node.order);
        ...
        while (pending.length) {
            ...
            //DBG("checking ancestor for "+current.id+":"+current.order);
            if (visited[current.id]){
              //DBG("already "+current.id+":"+current.order);
              continue;
            }
            // mark as visited
            ...
            if (current.id != node.id) {
                ...
                if (ancestorsIds.indexOf(current.id) == -1) {
                  ...
                  //DBG("already push "+current.id+":"+current.order);
                }
                
            }else{
              //DBG("current == node "+current.id+":"+current.order+" -- "+node.id+":"+node.order);
            }
            ...
            for (var i = 0; i < current.inputs.length; ++i) {
                ...
                if (opts.typesSkip && opts.typesSkip.length){
                    if (opts.typesSkip.indexOf(inputType) != -1){
                        ...
                        //DBG("type ok? "+input.id+":"+input.order+" :: "+inputType+" : "+opts.typesSkip.indexOf(inputType));
                    }
                }
                ...
                if (ancestorsIds.indexOf(input.id) == -1) {
                    if(!visited[input.id]){
                      ...
                      //DBG("already input "+input.id+":"+input.order);
                    }
                }
            }
        }
    ...
    LGraph.prototype.add = function(node, skip_compute_order, optsIn) {
        ...
        if (node.onAdded) {
            node.onAdded(this); // tag: node event entrypoint
        }
        ...
        if (this.onNodeAdded) {
            this.onNodeAdded(node); // tag: graph event entrypoint
        }
    ...
    LGraph.prototype.remove = function(node) {
        ...
        if (node.onRemoved) {
            node.onRemoved(); // tag: node event entrypoint
        }
    ...
    LGraph.prototype.change = function() {
        if (LiteGraph.debug) {
            console.log("Graph visual change");
        }
    ...
    */
    LGraph.prototype.onGraphChanged = function(optsIn) {
        ...
        if(opts.doSave && LiteGraph.actionHistory_enabled){
            ...
            if (LiteGraph.debug) {
                // console.debug("history action do not cache");
            }
        }
...
*/
    ...
    LGraphNode.prototype.configure = function(info) {
        ...
        if (this.graph) {
            this.graph.onGraphChanged({action: "nodeConfigure", doSave: false}); //this.graph._version++; // tag: graph event entrypoint
        }
    };
    ...
    LGraphNode.prototype.refreshAncestors = function(optsIn){
        ...
        //DBG(aAncestors.length + " ancestors for "+this.id+":"+this.order); // atlasan debug REMOVE
        for(iN in aAncestors){
            //DBG(aAncestors[iN].order + " node in ancestors"); // atlasan debug REMOVE
    ...
    LGraphNode.prototype.getInputData = function(slot, force_update, refresh_tree) {
         ...
         * */
        if (refresh_tree){
          //DBG("refresh tree!"); // atlasan debug REMOVE
            /*var optsAncestors = {  modesSkip: [LiteGraph.NEVER, LiteGraph.ON_EVENT, LiteGraph.ON_TRIGGER]
                                    ,modesOnly: [LiteGraph.ALWAYS, LiteGraph.ON_REQUEST]
                                    ,typesSkip: [LiteGraph.ACTION]
                                    ,typesOnly: []
    ...
    LGraphNode.prototype.addOnTriggerInput = function(){
        ...
        if (trigS == -1){ //!trigS || 
            var input = this.addInput("onTrigger", LiteGraph.EVENT, {removable: true, nameLocked: true}); //optional: true, nameLocked: true});
    ...
    LGraphNode.prototype.addOnExecutedOutput = function(){
        ...
        if (trigS == -1){ //!trigS || 
            var output = this.addOutput("onExecuted", LiteGraph.ACTION, {removable: true, nameLocked: true}); //optional: true, nameLocked: true});
	...
	LGraphNode.prototype.doExecute = function(param, options) {
        ...
        if (this.onExecute){
            ...
            this.onExecute(param, options); // event entrypoint or override
        ...
        if(this.onAfterExecuteNode) this.onAfterExecuteNode(param, options); // tag: event entrypoint
    };
	// retrocompatibility
    ...
    LGraphNode.prototype.actionDo = function(action, param, options, action_slot ) {
        ...
        if (this.onAction){
            ...
            this.onAction(action, param, options, action_slot); // event entrypoint or override
    ...
    LGraphNode.prototype.setSize = function(size)
	{
		this.size = size;
		if(this.onResize){
			this.onResize(this.size); // tag: event entrypoint
        }
	}
    ...
    LGraphNode.prototype.addOutput = function(name, type, extra_info) {
        ...
        if (this.onOutputAdded) {
            this.onOutputAdded(output); // event entrypoint or override
        }
    ...
    LGraphNode.prototype.addOutputs = function(array) {
        for (var i = 0; i < array.length; ++i) {
            ...
            this.outputs.push(o);
            if (this.onOutputAdded) {
                this.onOutputAdded(o); // event entrypoint or override
            }
    ...
    LGraphNode.prototype.removeOutput = function(slot) {
        ...
        this.setSize( this.computeSize() );
        if (this.onOutputRemoved) {
            this.onOutputRemoved(slot); // event entrypoint or override
        }
        this.setDirtyCanvas(true, true);
    };
    ...
    LGraphNode.prototype.addInput = function(name, type, extra_info) {
        ...
        if (this.onInputAdded) {
            this.onInputAdded(input); // event entrypoint or override
		}
    ...
    LGraphNode.prototype.addInputs = function(array) {
        for (var i = 0; i < array.length; ++i) {
            ...
            this.inputs.push(o);
            if (this.onInputAdded) {
                this.onInputAdded(o); // event entrypoint or override
            }
    ...
    LGraphNode.prototype.removeInput = function(slot) { /* , optsIn */
        ...
        this.setSize( this.computeSize() );
        if (this.onInputRemoved) {
            this.onInputRemoved(slot, slot_info[0] ); // event entrypoint or override
        }
        this.setDirtyCanvas(true, true);
    };
    ...
    */
    LGraphNode.prototype.getPropertyInfo = function( property )
	{
		...
		if (!info && this.onGetPropertyInfo) {
            info = this.onGetPropertyInfo(property); // event entrypoint or override
        }
    ...
    LGraphNode.prototype.getBounding = function(out, compute_outer) {
        ...
        if (this.onBounding) {
            this.onBounding(out); // event entrypoint or override
        }
        return out;
    };
    ...
    LGraphNode.prototype.connectByType = function(slot, target_node, target_slotType, optsIn) {
        ...
        if (target_slot >= 0 && target_slot !== null){
            ...
            //DBG("type "+target_slotType+" not found or not free?")
            ...
            if (LiteGraph.debug) {
                console.warn("no way to connect type: ",target_slotType," to targetNODE ",target_node);
            }
			//TODO filter
    ...
    LGraphNode.prototype.connectByTypeOutput = function(slot, source_node, source_slotType, optsIn) {
        ...
        if (source_slot >= 0 && source_slot !== null){
			...
			if (LiteGraph.debug) {
                console.warn("no way to connect byOUT type: ",source_slotType," to sourceNODE ",source_node);
            }
			//TODO filter
			
            //DBG("type OUT! "+source_slotType+" not found or not free?")
            return null;
        }
    }
    ...
    LGraphNode.prototype.connect = function(slot, target_node, target_slot) {
        ...
        if (target_slot.constructor === String) {
            ...
            if (LiteGraph.do_add_triggers_slots){
				...
				//DBG("Connect: Creating triggerEvent");
	            // force mode
        ...
        if (!this.outputs[slot]){
            if (LiteGraph.debug) {
                console.debug("Invalid slot passed: "+slot, this.outputs);
            }
            return null;
        }
        ...
        if (target_node.onBeforeConnectInput) {
            ...
            target_slot = target_node.onBeforeConnectInput(target_slot); // tag: event entrypoint
        }
        if (this.onConnectOutput) { // tag: event entrypoint
        ...
        //allows nodes to block connection, // tag: event entrypoint
    ...
    LGraphNode.prototype.disconnectOutput = function(slot, target_node, optsIn) {
        ...
        // in this method multi events for connecting and disconnecting on node and graph
        // onConnectionsChange // tag: node event entrypoint
        // onNodeConnectionChange // tag: graph event entrypoint
    ...
    LGraphNode.prototype.trace = function(msg) {
		...
		if(this.graph.onNodeTrace){
	        this.graph.onNodeTrace(this, msg); // tag: graph event entrypoint
        }
    };
...
*/
    ...
    DragAndScale.prototype.onMouse = function(e) {
		...
		//DBG("pointerevents: DragAndScale onMouse "+e.type+" "+is_inside);
    ...
    LGraphCanvas.prototype._doNothing = function doNothing(e) {
    	//DBG("pointerevents: _doNothing "+e.type);
        e.preventDefault();
        return false;
    };
    ...
    LGraphCanvas.prototype.unbindEvents = function() {
        ...
        //DBG("pointerevents: unbindEvents");
	...
	*/
    ...
    LGraphCanvas.prototype.processMouseDown = function(e) {
		...
		var y = e.clientY;
		//DBG(y,this.viewport);
		//DBG("pointerevents: processMouseDown pointerId:"+e.pointerId+" which:"+e.which+" isPrimary:"+e.isPrimary+" :: x y "+x+" "+y);
		...
		this.last_click_position = [this.mouse[0],this.mouse[1]];
		  ...
		  //DBG("pointerevents: pointer_is_double start");
		}else{
		  this.pointer_is_double = false;
		}
        ...
        if (e.which == 1 && !this.userInput_isNotPrimary)
		{
            ...
            if (!skip_action && clicking_canvas_bg && this.allow_dragcanvas) {
            	//DBG("pointerevents: dragging_canvas start");
			...
			if (LiteGraph.middle_click_slot_add_default_node){
				if (node && this.allow_interaction && !skip_action && !this.read_only){
					...
					if (
						...
						//DBG("middleClickSlots? "+mClikSlot+" & "+(mClikSlot_index!==false));
            	...
            	//DBG("pointerevents: dragging_canvas start from middle button");
            	this.dragging_canvas = true;
            }
	...
	*/
    ...
    LGraphCanvas.prototype.processMouseMove = function(e) {
        ...
        //DBG("pointerevents: processMouseMove "+e.pointerId+" "+e.isPrimary);
        
		if(this.block_click)
		{
			//DBG("pointerevents: processMouseMove block_click");
			e.preventDefault();
			return false;
		}
		...
		else if (this.selected_group && !this.read_only)
		{
        	...
        	////DBG("pointerevents: processMouseMove is dragging_canvas");
            ...
            if (this.node_dragged && !this.live_mode) {
				//DBG("draggin!",this.selected_nodes);
    ...
    LGraphCanvas.prototype.processMouseUp = function(e) {
    	...
    	if(!is_primary){
    		/*e.stopPropagation();
        	e.preventDefault();*/
    		//DBG("pointerevents: processMouseUp pointerN_stop "+e.pointerId+" "+e.isPrimary);
    		return false;
    	}
    	
    	//DBG("pointerevents: processMouseUp "+e.pointerId+" "+e.isPrimary+" :: "+e.clientX+" "+e.clientY);
		...
		if(!this.options.skip_events)
		{
			//DBG("pointerevents: processMouseUp adjustEventListener");
		...
		if(this.block_click)
		{
			//DBG("pointerevents: processMouseUp block_clicks");
			this.block_click = false; //used to avoid sending twice a click in a immediate button
		}

		//DBG("pointerevents: processMouseUp which: "+e.which);
        ...
        if (e.which == 1) {
            ...
            if (this.dragging_rectangle) {
				...
				if( this.onNodeMoved ){
					this.onNodeMoved( this.node_dragged ); // tag: node event entrypoint
                }
		...
		*/
        ...
        //DBG("pointerevents: processMouseUp stopPropagation");
        e.stopPropagation();
        e.preventDefault();
        return false;
    };
    ...
    LGraphCanvas.prototype.pasteFromClipboard = function(isConnectUnselected = false) {
        ...
        this.graph.onGraphChanged({action: "paste", doSave: true}); // tag: graph event entrypoint
    ...
    LGraphCanvas.prototype.checkDropItem = function(e) {
        if (e.dataTransfer.files.length) {
            ...
            if (nodetype) {
                ...
                if (node.onDropFile) {
                    node.onDropFile(file); // tag: node event entrypoint
                }
                this.graph.onGraphChanged({action: "fileDrop", doSave: true}); // tag: graph event entrypoint
				this.graph.afterChange();
            }
        }
    };
	...
	var clientX_rel = 0;
        ...
        //DBG("pointerevents: adjustMouseEvent "+e.clientX+":"+e.clientY+" "+clientX_rel+":"+clientY_rel+" "+e.canvasX+":"+e.canvasY);
    };
	...
	*/
    ...
    LGraphCanvas.showMenuNodeOptionalInputs = function(
        ...
        function inner_clicked(v, e, prev) {
            ...
            if (v.value) {
                ...
                if (node.onNodeInputAdd) { // tag: event entrypoint
    ...
    LGraphCanvas.showMenuNodeOptionalOutputs = function(
        ...
        function inner_clicked(v, e, prev) {
            ...
            if (
                ...
                if (node.onNodeOutputAdd) { // tag: event entrypoint
                ...
                case "Add Node":
					LGraphCanvas.onMenuAdd(null, null, e, menu, function(node){
						...
						if(!node.inputs || !node.inputs.length || !node.outputs || !node.outputs.length){
                            if (LiteGraph.debug) {
                                console.debug("Link Autoconnect missing something: node.inputs || !node.inputs.length || !node.outputs || !node.outputs.length",node.inputs,!node.inputs.length,!node.outputs,!node.outputs.length);
                            }
							return;
						}
			...
			if(typeof slotTypesDefault[fromSlotType] == "object" || typeof slotTypesDefault[fromSlotType] == "array"){
				for(var typeX in slotTypesDefault[fromSlotType]){
					if (opts.nodeType == slotTypesDefault[fromSlotType][typeX] || opts.nodeType == "AUTO"){
						...
						//DBG("opts.nodeType == slotTypesDefault[fromSlotType][typeX] :: "+opts.nodeType);
						break; // --------
					}
				}
			}else{
    ...
    LGraphCanvas.prototype.showConnectionMenu = function(optPass) { // addNodeMenu for connection
		...
		function inner_clicked(v,options,e) {
            //DBG("Process showConnectionMenu selection");
            switch (v) {
                case "Add Node":
                    LGraphCanvas.onMenuAdd(null, null, e, menu, function(node){
                        if (isFrom){
    ...
    LGraphCanvas.prototype.showSearchBox = function(event, options) {
            ...
            if (selIn){
                ...
                for (var iK=0; iK<nSlots; iK++){
                    ...
                    if(options.type_filter_in !==false && (options.type_filter_in+"").toLowerCase() == (aSlots[iK]+"").toLowerCase()){
                        //selIn.selectedIndex ..
                        opt.selected = true;
						//DBG("comparing IN "+options.type_filter_in+" :: "+aSlots[iK]);
	                }else{
						//DBG("comparing OUT "+options.type_filter_in+" :: "+aSlots[iK]);
					}
				}
		...
		*/
        ...
        function select(name) {
            if (name) {
                if (that.onSearchBoxSelection) {
                    that.onSearchBoxSelection(name, event, graphcanvas); // tag: method override
                } else {
                    ...
                    // !!
					// graphcanvas.graph.onGraphChanged({action: "nodeAdd", doSave: true}); // tag: graph event entrypoint
        ...
        function refreshHelper() {
            ...
            if (that.onSearchBox) {
                var list = that.onSearchBox(helper, str, graphcanvas); // tag: method override
                if (list) {
                    for (var i = 0; i < list.length; ++i) {
                        addResult(list[i]);
                    }
				...
				function inner_test_filter( type, optsIn )
				{
                    ...
                    if(options.do_type_filter && !opts.skipFilter){
                        ...
                        if(sIn && sV){
                            //DBG("will check filter against "+sV);
                        ...
                        if(sOut && sV){
                            //DBG("search will check filter against "+sV);
    ...
    LGraphCanvas.prototype.showEditPropertyValue = function( node, property, options ) {
        ...
        function setValue(value) {
            ...
            if (node.graph) {
                // node.graph._version++;
                node.graph.onGraphChanged({action: "propertyChanged", doSave: true}); // tag: graph event entrypoint
            }
            if (node.onPropertyChanged) {
                node.onPropertyChanged(property, value); // tag: event entrypoint
            }
			if(options.onclose){
				options.onclose();
            }
    ...
    LGraphCanvas.prototype.showShowGraphOptionsPanel = function(refOpts, obEv, refMenu, refMenu2){
        ...
        function inner_refresh(){
            ...
            // TODO add or pass objects so pass in title, options and implement combo and more types
    ...
    LGraphCanvas.prototype.showShowNodePanel = function( node )
	{
		...
		function inner_refresh()
		{
            ...
            for(var pName in node.properties)
			{
                ...
                // return false in onAddPropertyToPanel implementation to keep showing the default widget
                // tag: event entrypoint
			...
			if(node.onShowCustomPanelInfo){
				node.onShowCustomPanelInfo(panel); // tag: event entrypoint
            }

            panel.footer.innerHTML = ""; // clear
			panel.addButton("Delete",function(){
				if(node.block_delete){ // tag: node option
					return;
                }
    ...
    LGraphCanvas.prototype.showSubgraphPropertiesDialogRight = function (node) {
        ...
        //DBG("showing subgraph properties dialog");
        var that = this;
    ...
    LGraphCanvas.prototype.getCanvasMenuOptions = function() {
        ...
        if (this.getMenuOptions) {
            options = this.getMenuOptions(); // tag: graph event entrypoint
        } else {
            options = [
                {
                    content: "Add Node",
				...
				// { content: "Arrange", callback: that.graph.arrange }, // check this
        ...
        if (this.getExtraMenuOptions) {
            var extra = this.getExtraMenuOptions(this, options); // tag: graph event entrypoint
            if (extra) {
    ...
    LGraphCanvas.prototype.getNodeMenuOptions = function(node) {
        ...
        if (node.getMenuOptions) {
            options = node.getMenuOptions(this); // tag: node event entrypoint
        } else {
            options = [
                {
                    content: "Inputs",
        ...
        if (node.graph && node.graph.onGetNodeMenuOptions) {
            node.graph.onGetNodeMenuOptions(options, node); // tag: graph event entrypoint
        }
    ...
    function ContextMenu(values, options) {
		...
		LiteGraph.pointerListenerAdd(root,"up",
            function(e) {
			  	//DBG("pointerevents: ContextMenu up root prevent");
                e.preventDefault();
                return true;
            },
            true
        ...
        LiteGraph.pointerListenerAdd(root,"down",
            function(e) {
			  	//DBG("pointerevents: ContextMenu down");
                if (e.button == 2) {
                    that.close();
                    e.preventDefault();
                    return true;
		...
		LiteGraph.pointerListenerAdd(root,"enter", function(e) {
		  	//DBG("pointerevents: ContextMenu enter");
            if (root.closing_timer) {
                clearTimeout(root.closing_timer);
            }
        });
    ...
    ContextMenu.prototype.addItem = function(name, value, options) {
        ...
        element.do_click = inner_onclick;
	...
	LiteGraph.pointerListenerAdd = function(oDOM, sEvIn, fCall, capture=false) {
		if (!oDOM || !oDOM.addEventListener || !sEvIn || typeof fCall!=="function"){
			//DBG("cant pointerListenerAdd "+oDOM+", "+sEvent+", "+fCall);
			return; // -- break --
		}
	...
	LiteGraph.pointerListenerRemove = function(oDOM, sEvent, fCall, capture=false) {
		if (!oDOM || !oDOM.removeEventListener || !sEvent || typeof fCall!=="function"){
			//DBG("cant pointerListenerRemove "+oDOM+", "+sEvent+", "+fCall);
			return; // -- break --
		}
		switch(sEvent){
			//both pointer and move events
